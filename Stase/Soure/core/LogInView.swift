        ...
        VStack {
            ...
            Button(action: {
                ...
                HStack {
                    ...
                    .cornerRadius(10)
            }.alert(isPresented: $remote.isError){
                var show_description = "登录失败"
                if let login_description = self.remote.loginError {
                    show_description = login_description.error_description
                }
                
                return Alert(title: Text("错误"), message: Text(show_description), dismissButton: .default(Text("确定")))
